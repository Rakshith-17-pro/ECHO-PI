...
const Wiki = () => {
  ...
  const handleSearch = async () => {
    ...
    setIsSearching(true);
    try {
      // Navigate to local kiwix viewer with search query (ensure kiwix-serve is running)
      window.location.href = `http://10.139.20.130:8090/viewer#wikipedia/${encodeURIComponent(searchQuery)}`;
    } catch (error) {
      toast({
        title: "Search Failed",
